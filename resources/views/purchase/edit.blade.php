        ...
        <form action="{{ route('purchase.update', ['id' => $purchase->id]) }}" method="post">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="inventory_id" id="inventory">
                    ...
                    <option value="{{ $item->id }}" {{ $purchase->details->inventory->id === $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <x-text-input id="quantity" class="block mt-1 w-full" type="text" isNumber name="quantity" :value="old('quantity', $purchase->details->qty)" />
                <div class="form-text text-danger" attr-error-message="quantity"></div>
            </div>
            <div class="mb-3">
                <x-input-label for="date" :value="__('Date')" />
                <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $purchase->date)" />
